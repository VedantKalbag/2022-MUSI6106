...
//            std::cout << "buffer2[i]  " << buffer2[i] << std::endl;
    ...
    TEST_F(FastConv, timedomainidentity)
    {
        ...
        float delayedOutput_10[10];
        float convOutput_10[10];
        
        for (int i=0; i<10; i++)
        {
            delayedOutput_10[i] = delayedOutput[i];
            convOutput_10[i] = convOutput[i];
        }
        
        
        CHECK_ARRAY_CLOSE(delayedOutput_10, convOutput_10, 10, 0.0001f);
    ...
    TEST_F(FastConv, timedomainflushbuffer)
    {
        ...
        /*
        float TestImpulse[51] = { 0.f };
        float TestInput[10] = { 0.f };
        float TestOutput[10] = { 0.f };
        float TestFlush[50] = { 0.f };
        float CheckOutput[60] = { 0.f };
        TestInput[3] = 1;
        for (int i = 0; i < 51; i++)
        {
        ...
        m_pCFastConv->init(TestImpulse, 51, 2, CFastConv::kTimeDomain);
